...
namespace BankApp
{
    class Program
    {
        ...
        public static void Exit(List<Bank> Accounts, int managersPin)
        {
            ...
            if(manPin == managersPin)
            {
                try
                {
                    ...
                    StreamWriter sw = new StreamWriter("AccountInformation.txt");

                    TextInfo textInfo = new CultureInfo("en-US", false).TextInfo;

                    foreach (var item in Accounts)
                    {
                        sw.WriteLine($"{textInfo.ToTitleCase(item.AccountName)}|{item.AccountNum}|{item.AccountBalance}|{item.AccountPin}");
                    }
        ...
        public static bool Open(List<Bank> Accounts, List<int> AccountNums, int managerPin)
        {
            ...
            else
            {
                ...
                try
                {
                    StreamReader sr = new StreamReader("AccountInformation.txt");
                    ...
                    { 
                        ...
                        Accounts.Add(new Bank { AccountName = lines[0], AccountNum = Convert.ToInt32(lines[1]), AccountBalance = Convert.ToDouble(lines[2]), AccountPin = Convert.ToInt32(lines[3]) });
                        AccountNums.Add(Convert.ToInt32(lines[1]));
        ...
        {
            while ((Regex.IsMatch(uInput, @"^[0-9]{4}") == false))
            {
